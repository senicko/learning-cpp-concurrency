    ...
    private:
        ...
        bucket_iterator find_entry_for(Key const& key)
        {
            return std::find_if(data.begin(), data.end(),
                [&](bucket_value const& entry) { return entry.first == key; });
        }
...
int main()
{
    lookup_table::threadsafe_lookup_table<std::string, int> table;

    table.add_or_update_mapping("Hello", 10);
    assert(table.value_for("Hello") == 10);

    table.add_or_update_mapping("Hello", 20);
    assert(table.value_for("Hello") == 20);

    std::map<std::string, int> snapshot = table.get_map();
    assert(snapshot.size() == 1);
    assert(snapshot["Hello"] == 20);

    table.remove_mapping("Hello");
    assert(table.value_for("Hello", -1) == -1);
}
